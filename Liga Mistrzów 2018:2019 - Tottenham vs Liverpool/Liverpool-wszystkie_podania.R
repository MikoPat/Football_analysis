...
passes_Liverpool<-StatsBombData_Chl_2018_19%>%
  filter(type.name == "Pass", team.name == "Liverpool")

create_Pitch()+
  geom_point(data = passes_Liverpool, aes(x = location.x, y = location.y), colour = "red",alpha = 1.0)+
  geom_segment(data = passes_Liverpool, aes(x = location.x, y = location.y, xend = pass.end_location.x, yend = pass.end_location.y), alpha = 0.5, arrow = arrow(length = unit(0.07,"inches")))+ scale_y_reverse()
